/**
 * Fluxo E2E IP Connect (massa pronta): Oportunidade → Cotação → Viabilidade → Pedido → PEGA (designação/configuração/validação).
 *
 * Igual a `gerar-pedido-massa-pronta-ip-connect.js` até o subpedido “Em implantação”, depois:
 * GET `obterdadosordem` → PATCH `DesignarFacilidadeDados` → GET `obterdadosordem` → PATCH `ConfigurarDados`
 * (ver `support/utils/pega/` e o trace em fluxo-pega).
 *
 ...
 *   node scripts/gerar-pedido-massa-pronta-ip-connect-config-pega.js
 *
 * Modo cotação pronta (só gerar pedido a partir de cotação já existente):
 *   QUOTE_ID_READY=1 QUOTE_ID=0Q0xxx ACCOUNT_BUSINESS_ID=001xxx ACCOUNT_ORGANIZATION_ID=001xxx CONTACT_TECNICO_ID=003xxx node scripts/gerar-pedido-massa-pronta-ip-connect-config-pega.js
 *
 * Criar Lead e contas do zero (equivalente ao fluxo completo de gerar-pedido-ip-connect.js):
 *   CREATE_FROM_LEAD=1 node scripts/gerar-pedido-massa-pronta-ip-connect-config-pega.js
 *
 * PEGA (após subpedido em implantação): OAuth2 primeiro, depois obterdadosordem → DesignarFacilidade → ConfigurarDados
 *   → validação técnica (refresh + viewType=form) → se `IsEmAgendamento`: SelecaoDePeriodo → SelecaoDoSlot/refresh → SelecaoDoSlot?viewType=form (fluxo-pega.txt).
 *   Credenciais: `support/fixtures/user.json` → bloco **`pega`** por ambiente (ou raiz `pega`), ou env **`PEGA_CLIENT_ID`**,
 *   **`PEGA_CLIENT_SECRET`**, `PEGA_TOKEN_URL`, `PEGA_BASE_URL`, `PEGA_COOKIE`, `PEGA_BEARER_TOKEN` (prioridade sobre OAuth2).
 *   `PEGA_ORDEM_SERVICO` força o número da OS. Omitir PEGA: `SKIP_PEGA=1`. Só até validação técnica (sem agendamento): `SKIP_PEGA_AGENDAMENTO=1`.
 *   Datas do período (opcional): `PEGA_AGENDAMENTO_DATA_INICIO`, `PEGA_AGENDAMENTO_DATA_FIM` (ISO). Slot: `PEGA_AGENDAMENTO_SLOT_INDEX` (1 = primeiro).
 *   PATCHs de views do caso (`pyDetailsTabContent`, `DadosPedidoOSTab`): omitir com `SKIP_PEGA_CASE_VIEW_REFRESH=1`.
 *
