/**
 * Fluxo E2E Link Dedicado (massa pronta): Oportunidade → Cotação → Viabilidade → Pedido → PEGA (duas pontas).
 *
 * Igual a `gerar-pedido-massa-pronta-link-dedicado.js` até todos os subpedidos OK, depois PEGA na ordem do arquivo
 * `fluxo completo de link dedicado.txt`: Ponta A até ConfiguracaoDeRede → Ponta B Designar+Configurar → validação A/B
 * (somente ValidarExecucaoTecnica?viewType=form + views) → EVC ConfigurarEvc → agendamento Ponta A e depois Ponta B.
 *
 * Credenciais PEGA: `support/fixtures/user.json` → `pega`, ou `PEGA_CLIENT_ID` / `PEGA_CLIENT_SECRET` / `PEGA_BEARER_TOKEN`.
 * `SKIP_PEGA=1` omite PEGA. `SKIP_PEGA_AGENDAMENTO=1` afeta agendamento nas duas pontas. Overrides opcionais:
 * `PEGA_ORDEM_SERVICO_PONTA_A`, `PEGA_ORDEM_SERVICO_PONTA_B` (CRM OrderNumber dos subpedidos).
 * Se `obterdadosordem` da Ponta B voltar `[]`: `PEGA_LD_DELAY_BEFORE_PONTA_B_MS` (pausa antes do passo B; padrão 5000),
 * `PEGA_LD_OBTER_PONTA_B_MAX_TRIES` (padrão 30), `PEGA_LD_OBTER_PONTA_B_RETRY_MS` (padrão 4000).
 * ConfigRede 422 `D_AtvSavable` / ATV irmão ainda sem registro: retentativas GET+PATCH na mesma perna (`PEGA_LD_CONFIG_REDE_SIBLING_MAX_ROUNDS` padrão 5,
 * `PEGA_LD_CONFIG_REDE_SIBLING_RETRY_MS` padrão 4000), depois fallback Ponta B se `PEGA_ORDEM_SERVICO_PONTA_B` estiver definido.
 * Validação técnica LD: por defeito **sem** `OrdemServico.EscolasConectadas` (alinhado ao Postman). Para forçar: `PEGA_LD_VALIDAR_ORDEM_ESCOLAS=1`.
 *
 * Modo massa pronta:
 *   START_FROM_QUOTE=1 ACCOUNT_ORGANIZATION_ID=... ACCOUNT_BUSINESS_ID=... ACCOUNT_BILLING_ID=... node scripts/gerar-pedido-massa-pronta-link-dedicado-config-pega.js
 *
 * --- Documento vs automação (ler antes de assumir “igual ao Postman”) ---
 * O ficheiro `fluxo completo de link dedicado.txt` é a referência humana; este script segue a *sequência de fases* e os *endpoints*
 * principais, mas NÃO é cópia byte-a-byte: há GETs extra (refresh pyMemo, retries quando `[]`), pausa antes da Ponta B,
...
const PRODUCT_CODE_CREATE_QUOTE_MEMBERS_LD =
  process.env.PRODUCT_CODE_CREATE_QUOTE_MEMBERS_LD || 'LINK_DEDICADO';
...
async function fillAddressInfoWithTokenFallback(apiCall, addr, tokenCandidates, traceBase) {
  ...
  for (let idx = 0; idx < uniqueTokens.length; idx++) {
    ...
    //  PRIMEIRA TENTATIVA: Buscar só com CEP (como o frontend faz)
    const cepPayload = {
    ...
    if (cepRes.status === 200 && Array.isArray(cepRes.data) && cepRes.data.length > 0) {
      ...
      if (baseAddress) {
        ...
        // Retornar o endereço base, mas com o número do cliente
        return {
          addressInfo: {
            ...
            //  IMPORTANTE: Adicionar o número do cliente!
    ...
    // Fallback: tentar com o endereço completo (como seu script já faz)
    const fullPayload = {
...
async function resolvePointProduct(apiCall) {
  ...
  return {
    ...
    objectTypeName: rec.vlocity_cmt__ObjectTypeName__c //  ESSENCIAL
  };
}

function buildLdAttributes(pointType, speed, kind = 'access') {
  if (kind === 'evc') {
    return {
      ATT_ACCESSTYPE: 'VLAN-Based',
      ATT_NETWORKNAME: 'AUTOMACAO TESTE',
      ATT_TRANSPORTTYPE: 'Link Dedicado',
      ATT_PROTECAO: '1+0',
      ATT_TIPOVELOCIDADE: 'Simétrico',
      ATT_TOPOLOGY: 'Ponto a ponto',
      ATT_POINTTYPE: 'EVC',
      ATT_VELOCITY: String(speed.value),
      ATT_INSTALLATIONTYPE: 'Rede Nova',
      ATT_NETWORKTYPE: 'Determinístico',
      ATT_PRODUCT_CODE_SOV: PRODUCT_CODE_VIABILITY_LD
    };
  }
  return {
    ATT_Approach: 'Simples',
    ATT_NETWORKTYPE: 'Determinístico',
    ATT_INSTALLATIONTYPE: 'Rede Nova',
    ATT_VELOCITY: String(speed.value),
    ATT_POINTTYPE: pointType,
    ATT_TOPOLOGY: 'Ponto a ponto',
    ATT_TIPOVELOCIDADE: 'Simétrico',
...
function buildCreateQuoteMembersBody(quoteId, addr, addrB, ldProduct = null, selectedSpeed = null, addressInfoA, addressInfoB, pointProduct) {
  ...
  return {
    ...
    QuoteMemberList: [{
      ...
      //  ESTRUTURA CRÍTICA: Ponta A e Ponta B como objetos aninhados
      pointA: {
...
async function waitForLdViability(apiCall, userId, quoteId) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (state.hasRangeError) {
      // console.log(`[E2E] Viabilidade retornou erro interno de faixa no poll ${attempt}/${maxAttempts}. Seguindo para Query QuoteLineItem...`);
      // return lastRes;
...
/** Mesmo fluxo TRG → TI que `gerar-pedido-link-dedicado.js` (Reviewed → validity → Approved + tolerância TRG). */
...
// =============================
    ...
    if (orderId) {
      ...
      const pontaA = lastSubOrdersForPega.find((r) => (r.Vtal_Seg_PointType__c || '').trim() === 'Ponta A');
      const pontaB = lastSubOrdersForPega.find((r) => (r.Vtal_Seg_PointType__c || '').trim() === 'Ponta B');
      const evc = lastSubOrdersForPega.find((r) => (r.Vtal_Seg_PointType__c || '').trim() === 'EVC');
      return {
        quoteId,
        orderId,
        orderNumber,
...
/**
 * Link Dedicado: Organization e Billing precisam de vtal_LXD_FantasyName__c; massas sem ativação completa podem vir null e
 * impedir CreateOrderOnQuote / subpedidos. Preenche os dois com o mesmo valor quando algum estiver vazio.
 * Prioridade: LXD_FANTASY_NAME (env) → fantasy já na conta → Name da Org/Billing → fallback E2E-LD-timestamp.
 * TRG: mesmo com FantasyName ok nos dois, a Organization pode vir sem vtal_LXD_OrganizationName__c (obrigatório no TRG) —
 *   ao final faz GET de novo e aplica buildOrganizationPatchPayload quando o campo estiver vazio.
 */
async function ensureLxdFantasyNameOnOrgAndBilling(instanceUrl, accessToken, cookie, accountOrganizationId, accountBillingId) {
  const apiCall = (method, path, body) => api(instanceUrl, accessToken, method, path, body, cookie);
  const orgRes = await apiCall('GET', `${SOBJECTS_ACCOUNT}/${accountOrganizationId}`);
  const billRes = await apiCall('GET', `${SOBJECTS_ACCOUNT}/${accountBillingId}`);
  if (orgRes.status !== 200 || billRes.status !== 200) {
    console.warn('[E2E] ensureLxdFantasyName: GET Organization/Billing falhou; seguindo sem PATCH.');
    return;
  }
  let org = orgRes.data || {};
  const bill = billRes.data || {};
  const fromOrg = org.vtal_LXD_FantasyName__c && String(org.vtal_LXD_FantasyName__c).trim();
  const fromBill = bill.vtal_LXD_FantasyName__c && String(bill.vtal_LXD_FantasyName__c).trim();
  if (!(fromOrg && fromBill)) {
    const envOverride = process.env.LXD_FANTASY_NAME?.trim();
    const resolved =
      envOverride ||
      fromOrg ||
      fromBill ||
      (org.Name && String(org.Name).trim()) ||
      (bill.Name && String(bill.Name).trim()) ||
      `E2E-LD-${Date.now()}`;

    console.log('[E2E] START_FROM_QUOTE: vtal_LXD_FantasyName__c ausente na Organization e/ou Billing — preenchendo (LD)...');
    if (!fromOrg) {
      const patch = {
        vtal_LXD_FantasyName__c: resolved,
        vtal_LXD_OrganizationName__c: org.vtal_LXD_OrganizationName__c || resolved,
      };
      const r = await apiCall('PATCH', `${SOBJECTS_ACCOUNT}/${accountOrganizationId}`, patch);
      console.log('   Organization PATCH vtal_LXD_FantasyName__c:', r.status === 204 || r.status === 200 ? 'OK' : r.status);
    }
    if (!fromBill) {
      const r = await apiCall('PATCH', `${SOBJECTS_ACCOUNT}/${accountBillingId}`, { vtal_LXD_FantasyName__c: resolved });
      console.log('   Billing PATCH vtal_LXD_FantasyName__c:', r.status === 204 || r.status === 200 ? 'OK' : r.status);
    }
  }

  if (!IS_TRG) return;

  const orgFresh = await apiCall('GET', `${SOBJECTS_ACCOUNT}/${accountOrganizationId}`);
  if (orgFresh.status !== 200) return;
  org = orgFresh.data || {};
  const orgDisplayName = org.vtal_LXD_OrganizationName__c && String(org.vtal_LXD_OrganizationName__c).trim();
  if (orgDisplayName) return;

  const envOverrideTrg = process.env.LXD_FANTASY_NAME?.trim();
  const patch = buildOrganizationPatchPayload(envOverrideTrg || org.vtal_LXD_FantasyName__c, {
    accountName: org.Name || '',
    fallback: `E2E-LD-${Date.now()}`,
  });
  const resolvedOrg = patch.vtal_LXD_OrganizationName__c && String(patch.vtal_LXD_OrganizationName__c).trim();
  if (!resolvedOrg) {
    fail(
      'TRG: Organization sem vtal_LXD_OrganizationName__c e sem dados para derivar (Name / vtal_LXD_FantasyName__c / LXD_FANTASY_NAME).',
      { status: 0, data: org },
    );
  }
  console.log('[E2E] TRG: vtal_LXD_OrganizationName__c vazio na Organization — aplicando PATCH (massa pronta LD)...');
  const r = await apiCall('PATCH', `${SOBJECTS_ACCOUNT}/${accountOrganizationId}`, patch);
  if (r.status !== 200 && r.status !== 204) {
    fail('TRG: PATCH Organization (vtal_LXD_OrganizationName__c)', r);
  }
}
...
async function main() {
  ...
  if (!createFromLead && !readyQuote && !skipLead) {
    console.error('[E2E] Este script é massa pronta (Link Dedicado): defina as contas existentes, por exemplo:');
  ...
  if (readyQuote) {
    ...
    console.log('[E2E LD] Modo START_FROM_QUOTE: reutilizando massa. Só Opp → Cotação → Pedido (produto Link Dedicado).');
  } else if (createFromLead) {
    console.log('[E2E] Modo CREATE_FROM_LEAD: criando Lead e contas (como gerar-pedido-link-dedicado.js).');
  }
