/**
 * Fluxo E2E Link Dedicado (massa pronta): Oportunidade → Cotação → Viabilidade → Pedido.
 *
 * Este script é a variante Link Dedicado de gerar-pedido-massa-pronta-ip-connect.js:
 * - Usa produto Link Dedicado (ProductCode LINK_DEDICADO ou env PRODUCT_CODE_LD), não IP Connect.
 * - CreateQuoteMembers com duas pontas de endereço e produto Link Dedicado.
 * - Não chama IpConnectQuoteInstallationFee; usa VALOR_MENSAL_LD e VALOR_INSTALACAO_LD.
 * - ProductsValidation com FC_LDQuoteInstallationAddress preenchido.
 * - Vtal_SF_GetQuoteAddressViability com nomeProduto = PRODUCT_CODE_LD.
 *
 * Modo massa pronta (sem Lead/BRM — só Opp → Cotação → Pedido Link Dedicado):
 *   START_FROM_QUOTE=1 ACCOUNT_ORGANIZATION_ID=001xxx ACCOUNT_BUSINESS_ID=001xxx ACCOUNT_BILLING_ID=001xxx node scripts/gerar-pedido-massa-pronta-link-dedicado.js
 *   Contato técnico: se não informado CONTACT_TECNICO_ID, o script busca um da conta Business.
 *
 * Organization + Billing: vtal_LXD_FantasyName__c não pode ficar null no LD (pedido/subpedidos). O script preenche
 *   no fluxo Lead e, em START_FROM_QUOTE, faz PATCH se estiver vazio (override: LXD_FANTASY_NAME).
 * TRG: a conta Organization também precisa de vtal_LXD_OrganizationName__c; em START_FROM_QUOTE o script valida e
 *   aplica PATCH se estiver vazio (mesma prioridade que organizationPatchPayload: LXD_FANTASY_NAME → FantasyName → Name → fallback).
 *
